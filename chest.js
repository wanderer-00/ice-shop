...
function render(thisBtn, KIT) {
    ...
    // цикл создания ячеек
        ...
        if(KIT[i].MENU.length == 1) {
            ...
            for (a = 0; a < 27; a++) {
                cash += `<div class="cell" style="background-image: url(img/png/${KIT[i].MENU[0][0]}.png);"><div class="count">${KIT[i].MENU[0][1]}</div></div>`;
            }
            ...
            for (a = 0; a < 27; a++) {
                ...
                if( KIT[i].MENU[a][1] == 0){
                    ...
                    cash += `<div class="cell" style="background-image: url(img/png/${KIT[i].MENU[a][0]}.png);"><div class="count">${KIT[i].MENU[a][1]}</div></div>`;
                }            
            }
        };
